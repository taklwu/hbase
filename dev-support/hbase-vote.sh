...
Usage: ${SCRIPT} -s | --source <url> [-k | --key <signature>] [-f | --keys-file-url <url>]
       ${SCRIPT} -h | --help
...
function download_and_import_keys() {
    KEY_FILE_URL="${KEY_FILE_URL:-http://www.apache.org/dist/hbase/KEYS}"
...
function check_signatures() {
    ...
    gpg --verify hbase-${HBASE_VERSION}-src.tar.gz.asc hbase-${HBASE_VERSION}-src.tar.gz
    SIGNATURE=1
}

function check_checksums() {
    gpg --print-md SHA512 hbase-${HBASE_VERSION}-src.tar.gz > src.sha512
    diff src.sha512 hbase-${HBASE_VERSION}-src.tar.gz.sha512
    rm -f src.sha512

    gpg --print-md SHA512 hbase-${HBASE_VERSION}-bin.tar.gz > bin.sha512
    diff bin.sha512 hbase-${HBASE_VERSION}-bin.tar.gz.sha512
    rm -f bin.sha512
    CHECKSUM=1
}
...
function build_from_source() {
    mvn clean install -DskipTests && mvn test -P runAllTests
    BUILD_FROM_SOURCE=1
}
...
function print_when_exit() {
  cat << __EOF
        * Checksum : $(((${CHECKSUM})) && echo "ok" || echo "failed")
        * Signature: $(((${SIGNATURE})) && echo "ok" || echo "failed")
        * Rat check (${JAVA_VERSION}): $(((${RAT_CHECK})) && echo "ok" || echo "failed")
         - mvn clean apache-rat:check
        * Built from source (${JAVA_VERSION}): $(((${BUILD_FROM_SOURCE})) && echo "ok" || echo "failed")
         - mvn clean install -DskipTests
        * Unit tests pass (${JAVA_VERSION}): $(((${UNIT_TEST})) && echo "ok" || echo "failed")
         - mvn test -P runAllTests
__EOF
  if ((${CHECKSUM})) && ((${SIGNATURE})) && ((${RAT_CHECK})) && ((${BUILD_FROM_SOURCE})) && ((${UNIT_TEST})) ; then
    exit 0
  fi
  exit 1
}
